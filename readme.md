# MochiutaSC<br>(mochikara ura-net SCroll lyrics) v0.4

## はじめに
このツールは Music Video などのmp4動画に、スクロール歌詞を付けるものです。mp3にも対応しました！
...
3. 起動したら、まずダウンロードしたmp4ファイルを **ドロップ** してください。
すると **`曲の長さ`** 、**`出力ファイル`**  (assファイル)が入力されます。
    - 既にassファイルが存在する場合、その情報を曲情報と歌詞に表示します。

4. uta-net (https://www.uta-net.com/) でお目当ての曲を探します。
みつけたら、そのURLにある曲のID を **`uta-net ID`** の所に入力してください。
    - 例えばURLが https://www.uta-net.com/song/100001/ だったら、100001 を入力します。
    - **`ファイル名から検索`** を押すと、ブラウザを起動しドロップされたファイル名を基準に検索を行います。
    検索結果から **`uta-net ID`** を探して入力してください。
    検索には 株式会社フェンリル の Sleipnir Start を利用しています。(https://search.fenrir-inc.com/)

5. **`[歌詞取得]`** を押すと歌詞を取得し、曲情報と歌詞を表示します。
...
9. フォルダ内の前のファイルや次のファイルに遷移する場合は **`▲前`** **`▼次`** を押してください。
終了する場合は右上の **`×`** ボタンです。
...
これらは楽曲開始後に画面左上に30秒間表示されます。
    ...
    |`year`|リリース年|
    |`subdir`|画面表示はされませんが、格納するフォルダ名の判別に使用されます|
    |`作詞`|楽曲の作詞者名|
    |`作曲`|楽曲の作曲者名|
    |`編曲`|楽曲の編曲者名|
...
## 変更履歴
2026/07/03 v0.4 ファイル名から検索、フォルダ内の前/次のファイル に対応
2026/06/30 v0.3 mp3にもざっくり対応、動画は固定で8分のものを再生します
2026/06/08 v0.2 起動時にmp4を引数で受け付けるようにした、項目 subdir追加、kstyleの処理が誤っていたので修正
